...
plot(Set4.1.rp,branch = 0.4,uniform = T,margin = 0.1, # Fig. 8.15
...
spplot(thsn.sp, zcol = "Node", col.regions = greys,
   main = "Field 4.1 Recursive Partitioning Categories") # Fig. 9.16a

# Color version
color <- rainbow(6)
spplot(thsn.sp, zcol = "Node", col.regions = color,
   main = "Field 4.1 Recursive Partitioning Categories") 

thsn.sp$Yield <- data.Set4.1$Yield
greys <- grey(50:250 / 255)
spplot(thsn.sp, zcol = "Yield", col.regions = greys,
   main = "Field 4.1 Yield (kg/ha)") # Fig. 9.16b

# Color version
color <- terrain.colors(200)
spplot(thsn.sp, zcol = "Yield", col.regions = color,
