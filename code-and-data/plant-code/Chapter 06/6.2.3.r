...
data.Set4.sf <- st_as_sf(data.Set4, coords = c("Easting","Northing"))
data.Set4.sf$ID <- 1:nrow(data.Set4)
...
x.max <- max(st_coordinates(data.Set4.sf)[,1]) + 1
x.min <- min(st_coordinates(data.Set4.sf)[,1])
y.max <- max(st_coordinates(data.Set4.sf)[,2]) + 1
y.min <- min(st_coordinates(data.Set4.sf)[,2])
# Geneerate the ID number for each sub-region
x.index <- trunc(x.cells*(st_coordinates(data.Set4.sf)[,1] -
   x.min) / (x.max - x.min)) + 1
y.index <- trunc(y.cells*(st_coordinates(data.Set4.sf)[,2] -
   y.min) / (y.max - y.min)) + 1
data.Set4.sf$xyindex <- x.index + 10 * y.index
print(xyindex <- unique(data.Set4.sf$xyindex))
for (i in 1:length(xyindex)){
# Separate out points in sub-region i
   data.Set4.i <-
      data.Set4.sf[which(data.Set4.sf$xyindex == xyindex[i]),]
...
#   keep <- which(rowSums(matrix(as.numeric(mp$is_inf),
#      ncol = 6)) == 0)
   keep <- which(mp$is_inf == FALSE)
...
data.Set4.keep1 <- data.Set4.i[keep.id,]
