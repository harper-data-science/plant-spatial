...
ttest <- function(lambda, delta){
  Y <- IrWinv %*% rnorm(24^2)
  Ysamp <- matrix(Y, nrow = 24, byrow = TRUE)[3:22,3:22] + delta
...
U <- replicate(10000, ttest(lambda, delta))
...
t.b.boot <- function(lambda, delta){
...
nlist <- cell2nb(24, 24)
IrWinv <- invIrM(nlist, lambda)
U <- replicate(10000, t.b.boot(lambda, delta))
mean(U[1,]) # Error rate
...
t.para.boot <- function(IrWinv, nlist.small, W, delta){
# Generate the sample, eliminating the boundary
   Y <- IrWinv %*% rnorm(24^2)
   Y.samp <- matrix(Y, nrow = 24, byrow = TRUE)[3:22,3:22] + delta
...
U <- replicate(10000, t.para.boot(IrWinv, nlist.small, W, delta))
sum(U[1,]) / 10000 # Error rate
sum(U[2,]) / 10000 # Mean est. std. error
