...
# Papadakis method
trt.means <- tapply(plots.spdf@data$Yield,
  plots.spdf@data$trtmt.CRD, mean)
Yield.e <- plots.spdf@data$Yield -
    sapply(plots.spdf@data$trtmt.RCB, function(x) trt.means[x])

nb6x12 <- cell2nb(6,12)
W.papa <- nb2listw(nb6x12)
X <- listw2mat(W.papa) %*% Yield.e
papa.RCB <- with(plots.spdf@data, data.frame(Yield = Yield,
   trtmt = trtmt.RCB, X = X))
...
set.seed(123)
papa.form <- as.formula(Yield ~ trtmt + X)
U <- replicate(100, EMP.calc(papa.RCB, papa.form))
