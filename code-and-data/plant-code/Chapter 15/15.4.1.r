...
clusters.sf <- st_as_sf(clusters, coords = c("Easting", "Northing"))
plot(st_geometry(bdry.sf), axes = TRUE)
plot(st_geometry(clusters.sf), axes = TRUE, pch = clusters.sf$Cluster,
   add = TRUE)  # Fig. 15.10
...
# Color version
plot(st_geometry(bdry.sf), axes = TRUE)
plot(st_geometry(clusters.sf), axes = TRUE, pch = 16, col = clusters.sf$Color,
   add = TRUE)  # Fig. 15.10
title(main = "Field 4.1 Two Year Clusters", cex.main = 2,
    xlab = "Easting (m)", ylab = "Northing (m)", cex.lab = 1.5)
legend(592440, 4270600, c("Cluster 1", "Cluster 2", "Cluster 3", "Cluster 4"),
...
Easting <-clusters$Easting
Northing <- clusters$Northing
N.clus <- rep(Northing, 9)
E.clus <- rep(Easting, 9)
clus <- as.factor(sort(rep(1 : 9, length(Northing))))
aspect.r <- (max(Northing) - min(Northing)) / (max(Easting) - min(Easting))
xyplot(N.clus ~ E.clus | clus, aspect = aspect.r,
   main="Clusters", pch = 16, layout = c(3, 3),
   ylab="Northing", xlab="Easting", col = greys)
clus.num <- as.numeric(clusters[,1])
for(n in 2:9) clus.num <- c(clus.num, as.numeric(clusters[,n]))
clus.grey <- greys[clus.num]
xyplot(N.clus ~ E.clus | clus, aspect = aspect.r,
   main="Clusters", pch = 16, layout = c(3, 3),
   ylab="Northing", xlab="Easting", col = clus.grey)

# Color version
trellis.device(color = TRUE)
cols <- c("red", "green", "blue")
clus.col <- cols[clus.num]
xyplot(N.clus ~ E.clus | clus, aspect = aspect.r,
   main="Clusters", pch = 16, layout = c(3, 3),
   ylab="Northing", xlab="Easting", col = clus.col)
...
clusters2.sf <- st_as_sf(clusters2, coords = c("Easting", "Northing"))
plot(st_geometry(clusters2.sf), pch = clusters2.sf$X1) #Fig. 15.12a
title(main = "k = 2", cex.main = 2)


par(mai = c(1,1,1,1))
clusters.2mean <- matrix(nrow = 4, ncol = 2)  
for (i in 1:2) clusters.2mean[,i] <-
...
clusters3.sf <- st_as_sf(clusters3, coords = c("Easting", "Northing"))
plot(st_geometry(clusters3.sf), pch = clusters3.sf$X4) #Fig. 15.12b
title(main = "k = 3", cex.main = 2)

clusters.3mean <- matrix(nrow = 4, ncol = 3)
for (i in 1:3) clusters.3mean[,i] <-
...
clusters4.sf <- st_as_sf(clusters4, coords = c("Easting", "Northing"))
plot(st_geometry(clusters4.sf), pch = clusters4.sf$X2) #Fig. 15.12c
title(main = "k = 4, Cluster Set 1", cex.main = 2)
plot(st_geometry(clusters4.sf), pch = clusters4.sf$X3) # Almost the same
